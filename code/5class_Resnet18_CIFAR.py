...
parser.add_argument('--train_mode', type=str, default='all',help='Which part of network we are training: all, dense, conv, last_block')
...
class ResNet18(nn.Module):
    def __init__(self, num_classes=10, train_mode=train_mode):
        ...
        # Freeze or unfreeze parameters based on train_mode
        if train_mode == 'dense':
...
def evaluate_user_model_accuracy(model, testloader, device) -> float:
    """
    Evaluate the accuracy of a user's model on the test set.
...
for run in range(num_runs):
    ...
    for seed_index, seed in enumerate(seeds_for_avg):
        ...
        optimizer = optim.SGD((param for param in model.parameters() if param.requires_grad), lr=learning_rate, momentum=0.9)
        ...
        for timeframe in range(num_timeframes):
            ...
            for user_id in range(num_users):
                ...
                if selected_mode == 'user_selection_acc' or selected_mode == 'user_selection_acc_increment':
                    ...
                    with torch.no_grad():
                        for param, saved in zip(model.parameters(), w_after_train):
                            param.copy_(saved)
                    ...
                    val_dataset = torch.utils.data.TensorDataset(
                        torch.tensor(val_X).permute(0, 3, 1, 2).float() / 255.0,
                        torch.tensor(val_Y)
                    )
