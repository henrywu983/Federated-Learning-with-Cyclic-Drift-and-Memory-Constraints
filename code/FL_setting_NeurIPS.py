...
class FederatedLearning:
    ...
    # Compute cos similarities, convert to softmax probabilities, sample 3 users
    def simulate_fl_round_user_selection_softmax(self):
        """
        Selects users by sampling from a softmax over Lp-cosine similarity scores.

        Compared to simulate_fl_round_user_selection_cos (hard top-3), this policy:
          - Computes the same Lp-cosine score s_u for every user.
          - Converts the score vector into a probability distribution via
            softmax(beta * s), using the numerically stable max-subtraction trick.
          - Samples 3 distinct users without replacement using these probabilities.

        Properties:
          - When scores are tightly clustered (no signal), softmax probabilities
            are near-uniform and selection is effectively random.
          - When scores are spread out (strong signal), the policy concentrates on
            the highest-scoring users, approaching hard top-3 in the limit
            beta -> infinity.
          - beta is controlled by self.softmax_beta (set in __init__).
        """
        sum_terms = [torch.zeros_like(param).to(self.device) for param in self.w_before_train]
        packets_received = 0
        distinct_users = set()
        ...
        if self.slotted_aloha == 'true':
            ...
            num_distinct_users = len(distinct_users)
            print(f"Number of distinct clients: {num_distinct_users}")
            selected_users = list(distinct_users)

        else:
            if self.num_users < 3:
                raise ValueError("Number of users must be at least 3 to ensure proper selection.")

            # --- Compute Lp-cosine similarity scores for every user ---
            scores = np.zeros(self.num_users, dtype=np.float64)
            for user_id in range(self.num_users):
                user_grad_vector = torch.cat([g.view(-1) for g in self.grad_per_user[user_id]])
                global_grad_vector = torch.cat([g.view(-1) for g in self.prev_round_global_grad])
                scores[user_id] = self.lp_cosine_similarity(user_grad_vector, global_grad_vector,
                                                            p=self.cos_similarity)

            # --- Numerically stable softmax: subtract max before exponentiating ---
            z = self.softmax_beta * scores
            z = z - z.max()
            exp_z = np.exp(z)
            probs = exp_z / exp_z.sum()

            # --- Sample 3 users without replacement, weighted by softmax probs ---
            selected_users = list(np.random.choice(self.num_users, size=3, replace=False, p=probs))
            ...
            num_distinct_users = len(distinct_users)
            print(f"Number of distinct clients: {num_distinct_users} (No Slotted ALOHA, softmax beta={self.softmax_beta})")
    ...
    def run(self):
        ...
        elif self.mode == "user_selection_softmax":
            return self.user_selection_softmax()
    ...
    def user_selection_softmax(self):
        print(f"Running User Selection FL (Softmax over cosine, beta={self.softmax_beta})...")
        return self.simulate_fl_round_user_selection_softmax()
